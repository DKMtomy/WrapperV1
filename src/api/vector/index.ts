...
Vec3.magnitude = function magnitude(vec: Vector3) {
  return Math.sqrt(vec.x * vec.x + vec.y * vec.y + vec.z * vec.z);
};
Vec3.normalize = function normalize(vec: Vector3) {
  const l = Vec3.magnitude(vec);
  return Vec3(vec.x / l, vec.y / l, vec.z / l);
};
Vec3.cross = function crossProduct(a: Vector3, b: Vector3) {
  return Vec3(a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z, a.x * b.y - a.y * b.x);
};
Vec3.dot = function dot(a: Vector3, b: Vector3) {
  return a.x * b.x + a.y * b.y + a.z * b.z;
};
Vec3.angleBetween = function angleBetween(a: Vector3, b: Vector3) {
  return Math.acos(Vec3.dot(a, b) / (Vec3.magnitude(a) * Vec3.magnitude(b)));
};
Vec3.subtract = function subtract(a: Vector3, b: Vector3) {
  return Vec3(a.x - b.x, a.y - b.y, a.z - b.z);
};
Vec3.add = function add(a: Vector3, b: Vector3) {
  return Vec3(a.x + b.x, a.y + b.y, a.z + b.z);
};
...
Vec3.projection = function projection(a: Vector3, b: Vector3) {
  return Vec3.multiply(b, Vec3.dot(a, b) / (b.x * b.x + b.y * b.y + b.z * b.z) ** 2);
};
Vec3.rejection = function rejection(a: Vector3, b: Vector3) {
  return Vec3.subtract(a, Vec3.projection(a, b));
};
Vec3.reflect = function reflect(v: Vector3, n: Vector3) {
  return Vec3.subtract(v, Vec3.multiply(n, 2 * Vec3.dot(v, n)));
};
Vec3.lerp = function lerp(a: Vector3, b: Vector3, t: number) {
  return Vec3.multiply(a, 1 - t).add(Vec3.multiply(b, t));
};
Vec3.distance = function distance(a: Vector3, b: Vector3) {
