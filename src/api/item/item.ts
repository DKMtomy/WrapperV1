...
import { EnchantTypes } from '../types/item';
...
export class Item {
     ...
     */
    public addEnchantment(enchantment: string, level: number): boolean {
        if (!this.hasComponent('enchantments')) return false;

        const enchComp = this.getComponent('enchantments') as ItemEnchantsComponent;

        const enchants = enchComp?.enchantments;

        enchants.addEnchantment(new Enchantment(enchantment, level));
