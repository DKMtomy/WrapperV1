import { KeysOfCommandTypes } from '../types/Commands';
import { CommandExecState } from './CommandExecState';
import { Player as IPlayer } from '../player/index';
...
const isValidText = (text: string) => text.startsWith(QUOTE_CHARACTER) && text.endsWith(QUOTE_CHARACTER);
...
export namespace CommandTypes {
    ...
    export class Player {
        public static displayName = 'player';

        public static extract(a: CommandExecState): IPlayer | undefined {
            const text = a.readNext();

            if (typeof text === 'string') {
                if (!text.length) return this.extract(a);

                //check if the text is in between "" cus if not then return an error cus all player names must be in between ""
                if (!isValidText(text)) a.throwSyntaxError('Expected player name! (example: "player name"))');
...
export function checkObjectForCommandTypes(o: object): boolean {
    ...
    // Use Object.values to directly get enumerable own properties' values and then use Array.some
    return Object.values(CommandTypes).some(type => firstAssignment === type);
}
